...
} else # make comparisons----
{
  ...
  for (iVt in 1:(length(iV)-1))
  {
    ...
    { 
      ...
      system(paste0("/home/msheinman/Software/mummer-4.0.0rc1/nucmer --threads 1 --mum --prefix=",outdir,"/temp/Batch_",ind1,"_",ind2," ",listOfPairs$file1[i1]," ",listOfPairs$file2[i1]), intern = TRUE, wait=TRUE)
      ...
      alignment <- system(paste0("/home/msheinman/Software/mummer-4.0.0rc1/show-aligns -w ",1+max(c(Length1,Length2))," ",outdir,"/temp/Batch_",ind1,"_",ind2,".delta ", header1," ",header2), intern = TRUE, wait=TRUE)
